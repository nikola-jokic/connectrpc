...
use http::{HeaderMap, HeaderName, HeaderValue, header::AsHeaderName};
...
impl Metadata for HeaderMap {
    ...
    fn get_all_binary(
        ...
        let override_empty = !key.as_ref().ends_with(BIN_SUFFIX);
        get_all_maybe_trailer(self, key, override_empty).filter_map(|val| b64::decode(val).ok())
    }
...
fn binary_value(value: impl AsRef<[u8]>) -> HeaderValue {
    b64::encode(value).try_into().unwrap()
}
